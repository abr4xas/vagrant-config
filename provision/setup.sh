...
sudo apt-get update > /dev/null 2>&1
sudo apt-get upgrade -y > /dev/null 2>&1
echo "Installing node, npm & rethinkdb:..."
sudo aptitude install nodejs npm rethinkdb -y > /dev/null 2>&1
...
sudo echo "LC_ALL=en_US.UTF-8" >> /etc/default/locale > /etc/default/locale > /dev/null 2>&1
sudo gem install sass -y > /dev/null 2>&1
sudo apt-get autoremove -y > /dev/null 2>&1
sudo update-alternatives --skip-auto > /dev/null 2>&1
